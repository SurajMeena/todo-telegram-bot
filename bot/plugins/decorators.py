...
@bot_object.on_message(replied_msg,2)
...
@bot_object.on_message(except_replied_msg_and_commands,group=2)
            ...
            for i in hashtags:
                ...
                if hashtagtext in tracked_list:
                    ...
                    if(is_duplicate_item):
                        return
                    await bot_object.show_list_in_keyboard(todotype, chat_id, hashtagtext, message)
                else:
                    try:
                        await bot_object.send_message(chat_id, "Currently not tracking `{}` hashtag, Please use `/tracklists {}` if you want to track this hashtag list. You can also use `/ignore hashtag_names` to avoid this message next time".format(hashtagtext, hashtagtext), parse_mode="md")
                    except FloodWait as e:
                        await asyncio.sleep(e.x)
        else:
            db_ref = db.reference("/{}/{}".format(todotype, chat_id)).get()
            if(db_ref is not None):
                await bot_object.send_message(chat_id, "Currently not tracking any hashtag, Please use `/tracklists hashtag_names`, hashtag_names should be separated by commas", parse_mode="md")
            else:
                await bot_object.send_message(chat_id, "Please start todobot using /start@todogroup_bot")
    except Exception as e:
        logging.error(
            f"trouble in adding a msg containing hashtag into list [{chat_id}]", exc_info=True)


@bot_object.on_message(filters.command(["start", "start@todogroup_bot"], prefixes=["/", "/"]), group=1)
async def start_command(client, message):
    ...
    msg = await bot_object.create_list(todotype, message)
...
@bot_object.on_message(filters.command(["new", "new@todogroup_bot"]), group=1)
async def new_command(client, message):
    ...
    if(inline_msg_id is None):
        await bot_object.send_message(chat_id, "Please start todobot alteast once by using /start@todogroup_bot")
        return
    txt = txt.strip()
    if len(txt) != 0:
        ...
        if(inline_msg_id_node is None):
            await bot_object.send_message(chat_id, "Please use /start@todogroup_bot \
                command atleast once for using this bot")
        else:
            is_duplicate_item = addtodoitems(todotype, "newtodo", message)[1]
            if(is_duplicate_item):
                return
            await bot_object.show_list_in_keyboard(todotype, chat_id, "newtodo", message)


@bot_object.on_message(filters.command(["tracklists", "tracklists@todogroup_bot"]), group=1)
async def tracklist_handler(client, message):
    ...
    for listname in hashtags_lst:
        if len(listname) > 36:
            ...
            await bot_object.send_message(chat_id, "Fuck Off! What are you writing ? Ramayana")
        else:
            hash_str = "#" + listname
            ...
            if not bool(matched):
                ...
                await bot_object.show_list_in_keyboard(todotype, chat_id, "trackedlist", message)


@bot_object.on_message(filters.command(["ignore", "ignore@todogroup_bot"]), group=1)
async def ignore_handler(client, message):
    ...
    for i in hashtags_lst:
        if i in tracked_hashtags:
            await bot_object.send_message(chat_id, f"Uh-oh there is a clash, `{i}` is already in tracked lists")
        else:
            push_db(todotype, message, "ignore_lst", i)
    await bot_object.show_list_in_keyboard(todotype, chat_id, "ignore_lst", message)


@bot_object.on_message(filters.command(["showtrackedlists", "showtrackedlists@todogroup_bot"]), group=1)
async def tracked(client, message):
    ...
    if(todo_ref is None):
        await bot_object.send_message(chat_id, "Currently no hashtag is tracked, Use /start followed by `/tracklists hashtag1,hashtag2` for tracking hashtags", parse_mode="md")
        return
    tracked_hashtags = msg_list_from_db(todotype, chat_id, "trackedlist")[0]
    if(len(tracked_hashtags) == 0):
        await bot_object.send_message(chat_id, "Currently no hashtag is tracked, use `/tracklists hashtag1,hashtag2` for tracking hashtags", parse_mode="md")
    else:
        i = 1
...
@bot_object.on_callback_query(no_msg_in_cbq)
...
async def callback_handler(client, callback_query):
    ...
    if msg_id == bot_msg_id:
        if callbackdata in keys:
            ...
            lst_data = await bot_object.get_data(msg_list, from_usr)
            try:
                await bot_object.edit_message_text(chat_id, msg_id, "This is a hashtag **{}** list \n————————————————————\n".format(callbackdata) + lst_data, parse_mode="md", disable_web_page_preview=True)
                await bot_object.edit_message_reply_markup(
                    chat_id, msg_id,
            ...
            try:
                await bot_object.edit_message_text(chat_id, msg_id, "Click a task to delete or Click on a list name to open it")
                await bot_object.edit_message_reply_markup(
                    chat_id, msg_id,
            ...
            try:
                if(len((buttons+list_btns)[0]) == 0):
                    await bot_object.edit_message_text(chat_id, msg_id, "You don't have anything to delete. Please /start to use the bot")
                else:
                    await bot_object.edit_message_reply_markup(
                        chat_id, msg_id,
                        InlineKeyboardMarkup(buttons+list_btns)
